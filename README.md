...
Chunk 3 takes saves from chunk 2 and converts the large stitched data sets into acceleration by a flat transfer function determined by Hooke's Law and Newton's second law. Some initial analysis is completed here just to ensure the data appears healthy before creation of a .tsf (time series format) file. The tsf file can be loaded by TSoft for tidal model generation.
